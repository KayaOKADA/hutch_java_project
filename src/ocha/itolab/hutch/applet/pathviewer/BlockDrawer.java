    ...
    static void paintBlock(GL2 gl2, DataSet ds, int displayMode) {
		...
		for(int k = 19; k < 24; k++){
			...
			for(int i = 0; i < block.getNumBlocks(); i++) {
				...
				looktime = block.getLooktime();
				int view_floormap = 0;
			    if(looktime >= 19) view_floormap = looktime - 19;
			    else view_floormap = looktime + 5;
			  ...
			  //文字
		        ...
		        if(blocktime == looktime){
			        ...
			        gl2.glVertex3d(bpos[0][0], bpos[0][1], (float)view_floormap*(float)thick);
			        gl2.glVertex3d(bpos[1][0], bpos[1][1], (float)view_floormap*(float)thick);
			        gl2.glVertex3d(bpos[2][0], bpos[2][1], (float)view_floormap*(float)thick);
			        gl2.glVertex3d(bpos[3][0], bpos[3][1], (float)view_floormap*(float)thick);
			        gl2.glEnd();
		        }
			    ...
			    if(bb>1000){
			    	...
			    	int draw_sequence = 0;
			    	draw_sequence = blocktime-19;
			    	gl2.glColor4d(rr, gg, bb,transparency);
				    double vertex[][] = {
				    		  { bpos[0][0], bpos[0][1], ((float)draw_sequence)*thick },
				    		  { bpos[1][0], bpos[1][1], ((float)draw_sequence)*thick },
				    		  { bpos[2][0], bpos[2][1], ((float)draw_sequence)*thick },
				    		  { bpos[3][0], bpos[3][1], ((float)draw_sequence)*thick },
				    		  { bpos[0][0], bpos[0][1], ((float)draw_sequence+1)*thick },
				    		  { bpos[1][0], bpos[1][1], ((float)draw_sequence+1)*thick },
				    		  { bpos[2][0], bpos[2][1], ((float)draw_sequence+1)*thick },
				    		  { bpos[3][0], bpos[3][1], ((float)draw_sequence+1)*thick }
				    		};
				    int face[][] = {//面の定義
				    		  { 0, 1, 2, 3 },
				    		  { 0, 1, 5, 4 },
			        ...
			        */
			        ...
			        gl2.glColor4d(1.0, 1.0, 1.0, 1.0);
			        gl2.glBegin(GL2.GL_LINE_LOOP);
				    	gl2.glVertex3f(12.0f, -10.0f, (float)draw_sequence*(float)thick);
				    	gl2.glVertex3f(12.5f, -10.0f, (float)draw_sequence*(float)thick);
				    	gl2.glEnd();
				    	
				    	gl2.glRasterPos3d(12.8f, -10.0f, (float)draw_sequence*(float)thick);
		...
		gl2.glBegin(GL2.GL_LINE_LOOP);
		gl2.glVertex3f(-5.0f, -10.0f, 0.0f);
		gl2.glVertex3f(-5.0f, -10.0f, 7.0f);
		gl2.glVertex3f(12.0f, -10.0f, 7.0f);
		gl2.glVertex3f(12.0f, -10.0f, 0.0f);
		gl2.glEnd();
		...
		gl2.glBegin(GL2.GL_LINE_LOOP);
		gl2.glVertex3f(-5.0f, -10.0f, 7.0f);
		gl2.glVertex3f(-5.0f, 7.0f, 7.0f);
		gl2.glVertex3f(12.0f, 7.0f, 7.0f);
		gl2.glVertex3f(12.0f, -10.0f, 7.0f);
		gl2.glEnd();
		...
		if(looktime >= 19) looktime = looktime - 19;
		else looktime = looktime + 5;
		gl2.glDisable(GL2.GL_LIGHTING);
