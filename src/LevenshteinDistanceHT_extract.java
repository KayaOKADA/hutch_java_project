...
public class LevenshteinDistanceHT_extract {
	...
	static int datanum = 1001; //来客者数
	static String[] trajectory_s = new String[datanum]; //文字列格納

	public LevenshteinDistanceHT_extract(String filename, String t_filename) {
	...
	ArrayList<Character> TrajectoryList; //人流文字列リスト
	...
	private void stringReader() {
		BufferedReader t_br;
		this.TrajectoryList = new ArrayList<Character>();
		try {
			t_br = new BufferedReader(new FileReader(t_filename));
			String line = "";
			while ((line = t_br.readLine()) != null) {
				String[] oneline = line.split(",");//時間、文字列に分割
				this.TrajectoryList.add(oneline[1].charAt(0));
				trajectory_s[counter] = oneline[1];
				counter++;
			}
			System.out.println(Arrays.toString(trajectory_s));
			t_br.close();
		} catch (FileNotFoundException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public static void testSaveLabelData() {
		...
		LevenshteinDistanceHT_extract ldh = new LevenshteinDistanceHT_extract(label_file, trajectory_file);
	...
	public static void main(String[] args) throws UnsupportedEncodingException {
		String label_file = "LabelInfo_2_5_44_2.txt";//領域と距離を記録したテキストファイル
		String trajectory_file = "string_runlength.csv";
		LevenshteinDistanceHT_extract ld = new LevenshteinDistanceHT_extract(label_file, trajectory_file);

		ld.showRegionDistances();

		Double[][] result = new Double[datanum][datanum];
		for(int i=0;i<datanum;i++){
			for(int j=0;j<datanum;j++){
				//if((result[j][i] == null)&&(i!=j)){
				if(j>i){
					result[i][j] = ld.getWeightedLevenshteinDistance(trajectory_s[i], trajectory_s[j], Double.MAX_VALUE);
					System.out.println(trajectory_s[i] + ":" +trajectory_s[j] + " : "+ result[i][j]);
				}
			}
		}
