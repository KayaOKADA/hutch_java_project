...
datafile = open("/Users/okadakaya/Downloads/Hutch/Python/" + "clusteringInput.csv", "r")
    ...
    # Invoke Spectral Clustering
    features = np.array(fdata)
    sc = SpectralClustering(n_clusters=numc).fit(features)
    labels = sc.fit_predict(fdata)
    print(labels)
    
    # File output
    #labelfile = open(sys.argv[1] + "clusteringOutput.csv", "w")
labelfile = open("/Users/okadakaya/Downloads/Hutch/Python/" + "clusteringOutput.csv", "w")
    for label in labels:
